...
function getStatusClasses(status: PaymentStatus) {
  const classes: Record<PaymentStatus, string> = {
    PAID: "bg-[#E7F0E3] text-[#56704C]",
    PENDING: "bg-[#FFF3CD] text-[#866C22]",
    OVERDUE: "bg-[#F8E4E4] text-[#A05252]",
  };
...
export default function DoctorBalancePage() {
  ...
  const loadBalance = useCallback(async () => {
    try {
      ...
      setData(responseData as BalanceResponse);
    } catch (loadError) {
  ...
  const filteredPayments = useMemo(() => {
    ...
    return data.payments.filter((payment) => {
      ...
      return (
        ...
        budgetDescription.includes(normalizedSearch) ||
        ...
        <header className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-[#6B7774]">
              Consultá los cobros vinculados a tus presupuestos,
              pagos pendientes y actividad mensual.
            </p>
          </div>
          ...
          <>
            ...
            <section className="mt-6 grid gap-6 xl:grid-cols-[1.45fr_0.55fr]">
              <article className="border border-[#DED9CD] bg-white">
                ...
                {filteredPayments.length === 0 && (
                  <div className="px-6 py-16 text-center">
                    <ReceiptText className="mx-auto h-7 w-7 text-[#A2B38B]" />
                    ...
                    <p className="mt-2 text-sm text-[#6B7774]">
                      No se encontraron pagos para los
              ...
              <article className="border border-[#DED9CD] bg-white p-6">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#EEF2E9] text-[#6F855F]">
                  <WalletCards className="h-5 w-5" />
                </div>

                <p className="mt-6 text-xs font-semibold uppercase tracking-[0.2em] text-[#A2B38B]">
                  Resumen del período
                </p>
                ...
                <div className="mt-7 divide-y divide-[#DED9CD] border-y border-[#DED9CD]">
                  <SummaryRow
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <h2 className="mt-1 font-serif text-3xl">
                    Cobros mensuales de {selectedYear}
                  </h2>
                </div>
              </div>

              <div className="mt-10 flex h-72 items-end gap-4 overflow-x-auto pb-2">
...
function SummaryRow({
  ...
  return (
    <div className="flex items-center justify-between gap-5 py-4 text-sm">
      <span className="text-[#6B7774]">{label}</span>
