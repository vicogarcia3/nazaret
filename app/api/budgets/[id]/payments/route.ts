...
export async function GET(
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error: "No tenés permisos para consultar pagos.",
        },
        {
          status: 403,
        }
    ...
    if (session.user.role === "DOCTOR") {
      ...
      if (!doctor || doctor.id !== budget.doctorId) {
        return NextResponse.json(
          {
            error: "No tenés permisos para consultar estos pagos.",
          },
          {
            status: 403,
          }
    ...
    const remainingAmount = Math.max(budgetTotal - paidAmount, 0);
    ...
    console.error("Error obteniendo pagos del presupuesto:", error);
...
export async function POST(
  ...
  try {
    ...
    const result = await prisma.$transaction(async (transaction) => {
      ...
      if (session.user.role === "DOCTOR") {
        const sessionDoctor = await transaction.doctor.findUnique({
          where: {
            userId: session.user.id,
          },
          select: {
        ...
        if (!sessionDoctor || sessionDoctor.id !== budget.doctorId) {
      ...
      return {
        ...
        budget: {
          ...updatedBudget,
          subtotal: Number(updatedBudget.subtotal),
          discount: Number(updatedBudget.discount),
          total: Number(updatedBudget.total),
    ...
    if (
      error instanceof Error &&
      error.message === "BUDGET_COMPLETED"
    ) {
      return NextResponse.json(
        {
          error: "Este presupuesto ya está completamente pagado.",
        },
        {
          status: 400,
        }
    ...
    if (
      error instanceof Error &&
      error.message === "AMOUNT_EXCEEDS_REMAINING"
    ) {
      return NextResponse.json(
        {
          error: "El monto ingresado supera el saldo pendiente.",
        },
        {
          status: 400,
        }
